module termo_barostat
    ...
    subroutine berendsen_termostat(v,temp0,temp,delec,N,tau,dt)
        implicit none
        integer,intent(in)::N
        real(8),intent(in)::temp0,delec(3*N,2),tau,dt
    ...
    subroutine berendsen_barostat(pressure,pressure0,ni_bar,tau_bar,l,dt)
        implicit none
        real(8),intent(in)::pressure,pressure0,tau_bar,dt
        real(8),intent(inout)::ni_bar,l
        ni_bar=(1- dt/tau_bar *(pressure0 - pressure))**(1./3.)
        l=l*ni_bar
     ...
     integer,intent(in)::N
     real(8),intent(in)::dt,th_q,delec(3*N,2),temp0,v(3*N,3)
     ...
     integer,intent(in)::N
     real(8),intent(in)::delec(3*N,2),v(3*N,3)
    ...
    subroutine mtk_calc_th_eta(th_eta,dt,th_q,mtk_w,N,v,delec,temp0,mtk_nib)
        implicit none
        integer,intent(in):: N
        real(8),intent(in)::dt,th_q,mtk_w,v(3*N,3),temp0,delec(3*n,2),mtk_nib
        real(8),intent(inout)::th_eta
        real(8)::vs,nf
        integer::i
        nf=6*n-3
        ...
        integer,intent(in):: N
        real(8),intent(in):: virial,v(3*n,3),l,delec(3*n,2),x(3*n,2),a(3*n,3),dt,mtk_w,pressure0,th_eta
        real(8)::p,t   
       ...
       call calc_temp(v,N,delec,t)
        ...
        subroutine shift_coms(x,dt,mtk_nib,N,l,delec)
        real(8),intent(in)::dt,mtk_nib,l,delec(3*n,2)
        integer,intent(in)::N
        real(8),intent(inout)::x(3*n,3)
        integer::i
