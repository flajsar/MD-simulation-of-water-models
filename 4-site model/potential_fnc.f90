    ...
    subroutine lj_del_pot(x,delec,a,N,l,a1,a2,potencial_lj,virial)
        ...
        subroutine ewald_sum(x,delec,a,N,l,G,eps0,virial,potencial_el,k,self_int,r_om)

        implicit none
        integer, intent(in)::N,k
        real(8),intent(in)::x(4*N,3),delec(4*N,2),eps0
        real(8),intent(in)::l,G,self_int,r_om 
        ...
        real(8)::vec_k(3),rec_v(3,3),cntr_mass(3),d_i(4,3),force(3),mol(4,3),vs_re(4*N),vs_im(4*N)
        complex(8)::vs1,vs2,im,y1,t,vs1t
        
        allocate(f(4*N,4*N,3),vf(4*N,3))
        f=0
        vf=0
        pi=atan(1.)*4
        sqpi=dsqrt(pi)
        ...
        do i=1,4*N
            vf(i,1)=vf(i,1)+sum(f(i,:,1)) !sešteje sile da dobimo vsoto sil na atom i 
            vf(i,2)=vf(i,2)+sum(f(i,:,2))
            vf(i,3)=vf(i,3)+sum(f(i,:,3))
        end do
        ...
        do i=1,N
            ...
            d_i(4,:)=mol(4,:)-cntr_mass
            virial=virial+(dot_product(vf(i,:),d_i(1,:))+dot_product(vf(i+n,:),d_i(2,:))+dot_product(vf(i+n*2,:),d_i(3,:))+dot_product(vf(i+n*3,:),d_i(4,:)))
            
        end do
        
        call project_realf(vf,a,N,r_om,x,l) !preračuna sile iz 'dummy' atoma na realne
